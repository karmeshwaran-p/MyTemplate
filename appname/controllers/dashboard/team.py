...
def add_member(team_id):
    ...
    if form.validate_on_submit():
        if not team.billing_plan.can_add_more_users:
            flash(
                "You have exceeded the number of team members on your billing plan. Upgrade to continue",
                "warning",
            )
...
@blueprint.route(
    "/<hashid:team_id>/team/<hashid:invite_id>/remove_member", methods=["POST"]
)
@login_required
def remove_member(team_id, invite_id):
    ...
    if form.validate_on_submit():
        ...
        if len(team.active_members) <= 1 and team_member.email in existing_members:
            flash(
                "Teams must have at least one active user. You cannot remove the last active user",
                "warning",
            )
            return redirect(url_for(".index", team_id=team_id))
