...
class InviteEmail(Mailer):
    ...
    def __init__(self, invite):
        self.recipient = None
        self.invite = invite
        self.recipient_email = invite.invite_email or (
            invite.user and invite.user.email
        )

    @property
    def subject(self):
        return "{0} invited you to join their team on {1}".format(
            self.invite.inviter.email, branding.name
        )
    ...
    def send(self):
        link = url_for(
            ...
            secret=self.invite.invite_secret,
            _external=True,
        )
