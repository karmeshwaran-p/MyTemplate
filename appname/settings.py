...
class Config(object):
    ...
    GOOGLE_OAUTH_CLIENT_SECRET = os.getenv(
        "GOOGLE_CONSUMER_SECRET", "bad_secret_replace_me"
    )
    ...
    STORAGE_CONTAINER = os.getenv(
        "STORAGE_CONTAINER", os.path.abspath(__file__ + "/../../tmp")
    )  # bucket name or cloud
...
class DevConfig(Config):
    ...
    STORAGE_SERVER = (
        True  # Whether or not to expose the files over an endpoint (LOCAL only)
    )
    STORAGE_SERVER_URL = (
        "/dev-files"  # The url endpoint to access files on LOCAL provider
    )
