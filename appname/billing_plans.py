...
class BasePlan:
    ...
    def record_change_in_usage(self):
        ...
        if self.billing_type == METERED:
            ...
            all_subscription_items = stripe.all_subscription_items(
                self.team.subscription_id
            )
            subscription_items_for_this_plan = [
                s
                for s in all_subscription_items
                if s["product"] == self.stripe_product_id
            ]
            if len(subscription_items_for_this_plan) < 1:
                logger.debug(
                    "Failed on subscription ID {}".format(self.team.subscription_id)
                )
            ...
            logger.debug(
                "Setting subscription for team {} to {}".format(self.team.id, quantity)
            )
...
class MonthlyPremium(BasePlan):
    stripe_product_id = os.getenv(
        "STRIPE_MONTHLY_PREMIUM_ID", "price_1HGzFQID8JASalnlgBAX2hjo"
    )
...
class AnnualPremium(MonthlyPremium):
    stripe_product_id = os.getenv(
        "STRIPE_ANNUAL_PREMIUM_ID", "price_1HGzFQID8JASalnlBeRxlkno"
    )
    name = "annual_premium"
...
class MeteredPlan(BasePlan):
    stripe_product_id = os.getenv(
        "STRIPE_METERED_PLAN_ID", "price_1HGzFQID8JASalnlBeRxlkno"
    )
    billing_type = METERED
    name = "pay_as_you_go"
